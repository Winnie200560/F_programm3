...
указанном каталоге.
### Алгоритм решения
1. Пользователь вводит путь к каталогу.
2. Программа проверяет существует ли каталог с помощью Directory.Exists().
3. Далее получаем последовательность файлов (с полным путем к нему) с помощью Directory.EnumerateFiles().
4. Из полного пути извлекаем имя файла с помощью Path.GetFileName.
5. Файлы фильтруются по первой букве имени файла (Seq.filter).
6. Подходящие файлы выводятся на экран.
