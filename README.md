...
указанном каталоге.
### Алгоритм решения
1. Пользователь вводит путь к каталогу с помощью функции inputDirectory.
3. Пользователь вводит начальный символ для поиска файлов с помощью функции inputSymbol.
4.Программа проверяет, существует ли указанный каталог с помощью Directory.Exists. Если каталог не существует, выводится сообщение "Каталог не найден".
5. При существующем каталоге программа получает перечисление файлов с помощью Directory.EnumerateFiles.
6. Для каждого файла извлекается только имя файла (без пути) с помощью Path.GetFileName.
7. Выполняется фильтрация файлов, у которых имя начинается с заданного символа, используя String.StartsWith.
8. Подходящие имена файлов выводятся на экран с помощью Seq.iter.
