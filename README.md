...
2. Пользователь вводит начальный символ для поиска файлов с помощью функции inputSymbol.
3. Программа проверяет, существует ли указанный каталог с помощью Directory.Exists. Если каталог не существует, выводится сообщение "Каталог не найден".
4. При существующем каталоге программа получает перечисление файлов с помощью Directory.EnumerateFiles.
5. Для каждого файла извлекается только имя файла (без пути) с помощью Path.GetFileName.
6. Выполняется фильтрация файлов, у которых имя начинается с заданного символа, используя String.StartsWith.
7. Подходящие имена файлов выводятся на экран с помощью Seq.iter.
